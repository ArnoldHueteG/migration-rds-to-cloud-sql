...
def test_connection(source_connection, logger):
    ...
    try:
        ...
        conn.close()
        logger.debug(
            "Your connection to {} was successful.".format(source_connection["host"])
        )
        return True
...
def create_connection_profile(
    project_id, region_id, request_dict, wait_to_be_ready=False, logger=None
):
            ...
            response = (
                ...
                .get(
                    name="projects/{}/locations/{}/connectionProfiles/{}".format(
                        project_id, region_id, connection_profile_id
                    )
                )
                .execute()
            )
...
def create_migration_job(
    migration_job_id, project_id, region_id, request_dict, logger=None
):
    dms = discovery.build("datamigration", "v1")
    response = request_dict
    if not debug_mode:
            ...
            response = (
                ...
                .get(
                    name="projects/{}/locations/{}/migrationJobs/{}".format(
                        project_id, region_id, migration_job_id
                    )
                )
                .execute()
            )
...
def generate_migration_job(
    ...
    #################################################################################
    prefix_cp_source, prefix_cp_cloudsql, prefix_mj, now_str = (
        prefix_dict["prefix_cp_source"],
        prefix_dict["prefix_cp_cloudsql"],
        prefix_dict["prefix_mj"],
        prefix_dict["now_str"],
    )
    id = prefix_dict["id"]
    project_id, region_id = location_dict["project_id"], location_dict["region_id"]
    rds_name = source_connection["postgresql"]["host"].split(".")[0]
    connection_profile_id_source = "{1}{0}-{2}-{3}".format(
        id, prefix_cp_source, rds_name, now_str
    )
    connection_profile_id_cloudsql = "{1}{0}-{2}-{3}".format(
        id, prefix_cp_cloudsql, rds_name, now_str
    )
    migration_job_id = "{1}{0}-{2}-{3}".format(id, prefix_mj, rds_name, now_str)
    logger = setup_logger(
        "logs/{3}-{1}{0}-{2}.log".format(id, prefix_mj, rds_name, now_str)
    )
    ...
    create_connection_profile(
        **location_dict,
        ...
        wait_to_be_ready=False,
        logger=logger
    )
    request_dict = {
        "displayName": connection_profile_id_cloudsql,
        "cloudsql": {
            "settings": {
                **target_base_settings_cloud_sql,
                **target_server_settings_cloud_sql,
                "sourceId": "projects/{}/locations/{}/connectionProfiles/{}".format(
                    project_id, region_id, connection_profile_id_source
                ),
            }
        },
    }
    # logger.debug(json.dumps(request_dict, indent=2))
    create_connection_profile(
        **location_dict, request_dict=request_dict, wait_to_be_ready=True, logger=logger
    )
    request_dict = {
        "type": "CONTINUOUS",
        ...
        "destination": "projects/{}/locations/{}/connectionProfiles/{}".format(
            project_id, region_id, connection_profile_id_cloudsql
        ),
...
def get_state_from_migration_job(prefix_dict, location_dict, source_connection):
